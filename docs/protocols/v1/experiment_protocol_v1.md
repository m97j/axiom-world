...
- Protocol version: `v1.4` (v1.0 text preserved; changes appear only as marked amendment notes + §13 log)
...
| v1.2    | 2026-08-06 | §5.1, adapter contract    | B1/B2 SFT arms re-run as B1'/B2' with`adapter.modules_to_save: [lm_head, embed_tokens]` (new AdapterConfig field); original B1/B2 runs retained as negative results.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Termination failure audit (x09–x13): terminal `<                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| v1.3    | 2026-08-13 | §5.1 data governance, §8 | **Deviation D1 + remediation.** The PlayWorld SFT builder is deterministic only *per code commit*: `prompt_fingerprint` is stable across all builds (cc2aef0d), but 53/2000 oracle demonstrations flip their target among equally-optimal BFS paths across commits (x15). x16 lineage-fingerprint audit shows A1 and B4 each trained on *unfrozen in-session builds* matching none of the persisted HF artifacts — A1's exact training bytes are unrecoverable, so the B4-vs-A1 single-variable comparison cannot be restored retroactively. Remediation: (1) pin ONE canonical frozen artifact — `m97j/aw-playworld:train/v1/playworld_sft.jsonl` (fingerprint 042eb078, built at the current frozen code commit) — for ALL subsequent PlayWorld SFT training; (2) retrain **A1v2 and B4v2** on this artifact under their original recipes and rebase the RQ1 comparison (and A2v2/B5 lineage) on the v2 arms; (3) new standing rule: every training/eval dataset is built once, frozen to HF, and consumed only via sha-pinned `fetch_dataset` (`--expected-sha256`); `run_experiment` hard-fails on `data.source.fingerprint` mismatch. v1 A1/A2/B4 runs are retained as deviation baselines; all frozen-suite evaluations remain valid (freeze_fingerprint 3cdcbc30 matched on every eval run). |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| v1.4    | 2026-08-17 | §5.2, §8, §9, G6        | **Ablation descope + 3-seed final scope definition.** (1) Ablations E-RLOO, E-RANDPAIR, E-QLORA, E-ATTN (long-seq points), E-RULE and the LogosP policy-breadth study are **descoped** and moved to Future Work (§8 of the report). The pre-registered Holm–Bonferroni comparison family shrinks accordingly to {A2−B5, B5−B6, Bchamp−C1, Bchamp−C2}; the two dropped comparisons (B6−E-RLOO, B5−E-RANDPAIR) are disclosed as not run. (2) Gate G6's "3-seed confirmation" is scoped as follows: seeds **{42, 43, 44}**; the reseeded unit is the **final-stage Phase-2 training** of the champion arm **B4v2** and its Track-A control **A2v2**, each re-trained on the identical frozen data artifact and identical sha-pinned parent adapters (P1 champion for B4v2, A1v2 for A2v2, both fixed at their s42 runs-of-record), followed by full frozen-suite evaluation (greedy decoding). Upstream-stage seed variance is therefore NOT measured and is disclosed as a limitation. Headline claim requires sign consistency of the B4v2−A2v2 per-suite deltas across all 3 seeds; mean ± sd reported per §8.                                                                                                                                                                      | Budget rule (§9 cut order: E-QLORA → E-ATTN → E-RULE → LogosP) combined with the §6 B6/B6-R closure (docs/experiments/b6_grpo_closure.md): both GRPO arms failed via advantage starvation + entropy collapse → required-component specialization on the narrow training scenario pool — a data/regularization failure mode that RLOO shares (identical group-relative advantage structure), giving E-RLOO a low prior of changing any conclusion; E-RANDPAIR is moot because its target contrast (B5 mining quality) is anchored to a ~null B5 effect. Remaining GPU budget is reallocated to the never-cut 3-seed final. No frozen suite, decoding profile, or verifier is touched. |
